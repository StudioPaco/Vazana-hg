...
interface AvailabilityResult {
  ...
  warnings: string[]
  busyDates: string[] // dates where selected resources are booked (next 30 days)
}
...
export function useResourceAvailability(
  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (!date && selectedWorkerIds.length === 0 && selectedVehicleIds.length === 0) {
        setResult({ unavailableWorkerIds: new Set(), unavailableVehicleIds: new Set(), warnings: [], busyDates: [] })
        return
      }
      ...
      if (date) {
        ...
        const { data: dateJobs } = await supabase
          .from('jobs')
          .select('worker_id, vehicle_id, worker_name, vehicle_name, job_number, client_name')
          .eq('job_date', date)
          .eq('is_deleted', false)
      ...
      setResult({ unavailableWorkerIds: unavailableWorkers, unavailableVehicleIds: unavailableVehicles, warnings, busyDates })
    }

    const timeout = setTimeout(check, 300) // debounce
    return () => clearTimeout(timeout)
  }, [date, shiftType, workers, vehicles, selectedWorkerIds, selectedVehicleIds])
