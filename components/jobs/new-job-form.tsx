...
export default function NewJobForm({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const requiredFields = [
      ...
      { field: formData.employee || (additionalWorkers.length > 0 ? "has_additional" : ""), name: "עובד", key: "employee", message: "בחר עובד לעבודה" },
      { field: formData.vehicle || (additionalVehicles.length > 0 ? "has_additional" : ""), name: "רכב", key: "vehicle", message: "בחר רכב לעבודה" },
    ]
    ...
    try {
      ...
      const jobData = {
        ...
        worker_name: selectedEmployee?.name || additionalWorkers[0]?.name || "",
        worker_id: selectedEmployee?.id || additionalWorkers[0]?.id || null,
        vehicle_name: selectedVehicle ? `${selectedVehicle.license_plate} - ${selectedVehicle.name}` : additionalVehicles[0]?.name || "",
        vehicle_id: selectedVehicle?.id || additionalVehicles[0]?.id || null,
        cart_name: selectedCart?.name || additionalCarts[0]?.name || null,
        cart_id: selectedCart?.id || additionalCarts[0]?.id || null,
